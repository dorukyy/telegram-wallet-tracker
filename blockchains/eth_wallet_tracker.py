...
class ETHWalletTracker:
    def __init__(self):
        ...
        self.db = DBHandler()
        self.api_key = config.get("ETH","token")
    ...
    def get_wallet_transactions(self, wallet_address):
        url = f"https://api.etherscan.io/api?module=account&action=txlist&address={wallet_address}&sort=desc&apikey={self.api_key}"
        response = requests.get(url)
        data = response.json()
    ...
    def getNewTransactions(self):
        wallets = self.db.load_wallets_by_blockchain("eth")
        new_transactions = []
                ...
                for transaction in transactions:
                    ...
                    if transaction_date is not None and transaction_date > last_check:
                        try:
                            ...
                            message = self.eth_message_parser(data)
                            data["message"] = message

                            new_transactions.append(data)
                            self.db.update_last_check("eth", chat_id, wallet_address, transaction_date)
                        except KeyError:
                            print("Unexpected data type")

        return new_transactions

    def eth_message_parser(self, message_input):
        ...
        message = f"{title} \n" \
                  ...
                  f"Value: {message_input['value']} ETH \n"
        return message
